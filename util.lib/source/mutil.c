...
void dtri2sym( char uplo, int lda, double * tri, double * sym)
{
   ...
   */
   ...
   for(k = 0 ; k < lda; k ++ )
   {
     for(j=0;j<=k;j++)
     {
       //printf("\n[j = %d ; k = %d] [ number = % 10.6E ]\n" , j , k , *( tri + k *( k + 1 ) / 2 + j ) ) ;
	     
       *( sym + k * lda + j ) = *( tri + k *( k + 1 ) / 2 + j );

       *( sym + j * lda + k ) = *( sym + k * lda + j ) ;
  ...
  else if ( (uplo == 'U') || ( uplo == 'u') )
  {
   ...
   for(k=0;k<lda;k++)
   {
     for(j=k;j<lda;j++)
     {
       *( sym + k * lda + j ) = *( tri + ( 2 * lda - k + 1 ) * k / 2 + ( j - k ) ) ;
...
void dtranspose_nonsquare( int nrow , int ncol , double *porigin, double *ptarget )
{
  ...
  double * temp = calloc( nrow * ncol , sizeof(double) );
  
  for( irow = 0 ; irow < nrow ; irow ++ )
  {
    for( icol = 0 ; icol < ncol ; icol ++ )
    {
      *( temp + icol * nrow + irow ) = *( porigin + irow * ncol + icol );
    }
    
  }
  
  int irow_new , icol_new ;
  
  int nrow_new = ncol ; 
  
  int ncol_new = nrow ;
  
  for( irow_new = 0 ; irow_new < nrow_new ; irow_new ++ )
  {
    for( icol_new = 0 ; icol_new < ncol_new ; icol_new ++ )
    {
      *( ptarget + irow_new * ncol_new + icol_new ) = *( temp + irow_new * ncol_new + icol_new ) ;
...
double dmaxAbs( int length , double * sample )
{
  ...
  double tmp = fabs( *sample ) ;
  
  for( j = 1; j < length ; j ++ )
  {
    if( tmp <= fabs( *( sample + j ) ) )
...
int dmaxID( int length , double * sample )
{
  ...
  for( j = 1 ; j < length ; j ++ )
  {
    if( tmp <= ( *( sample + j ) ) )
    { 
      //printf("\nPrevious max = %lf ; Current max = %lf ; ID = %d ...\n" , tmp , *( sample + j ) , j ) ;
...
double hermite( int n , double x )
{
    switch ( n )
    {
      case 0 : return( 1.00 ) ; break ;
      case 1 : return( 2.00 * x ) ; break ;
      default : return( 2.00 * x * hermite( n-1 , x ) - 2.00 * ( n - 1 ) * hermite( n-2 , x ) ) ; break ;
